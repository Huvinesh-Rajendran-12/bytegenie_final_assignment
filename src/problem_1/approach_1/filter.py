...
class DataframeFilter:
    ...
    def filter_dataframes(self, conditions: Dict[str, Dict[str, Union[List, Dict]]]) -> Dict[str, pl.DataFrame]:
        ...
        while filter_queue:
            ...
            if not filtered_dfs[df_name].equals(initial_df):
                self._propagate_filter(df_name, filtered_dfs[df_name], filtered_dfs, filter_queue)

        filtered_dfs = self._ensure_consistency(filtered_dfs)

        return filtered_dfs

    def _apply_filter(self, df: pl.DataFrame, condition: Dict[str, Union[List, Dict]]) -> pl.DataFrame:
    ...
    def _apply_date_range_filter(self, df: pl.DataFrame, column: str, date_range: Dict[str, str]) -> pl.DataFrame:
        ...
        end_date = datetime.strptime(date_range.get('end', '9999-12-31'), '%Y-%m-%d').date()
        return df.filter((pl.col(column) >= start_date) & (pl.col(column) <= end_date))

    def _propagate_filter(self, source_df_name: str, filtered_df: pl.DataFrame, filtered_dfs: Dict[str, pl.DataFrame], filter_queue: deque):
        for (df1, df2), common_col in self.relationships.items():
            if df1 == source_df_name:
                ...
                filtered_dfs[df2] = filtered_dfs[df2].filter(pl.col(common_col).is_in(related_values))

                if not filtered_dfs[df2].equals(initial_df):
                    filter_queue.append((df2, {common_col: related_values}))

    def _ensure_consistency(self, filtered_dfs: Dict[str, pl.DataFrame]) -> Dict[str, pl.DataFrame]:
        for (df1_name, df2_name), common_col in self.relationships.items():
            ...
            filtered_dfs[df1_name] = filtered_dfs[df1_name].filter(pl.col(common_col).is_in(common_values))
            filtered_dfs[df2_name] = filtered_dfs[df2_name].filter(pl.col(common_col).is_in(common_values))
